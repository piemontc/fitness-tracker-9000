...
@Component({
  selector: 'app-stats-rower-miles',
  templateUrl: './stats-rower-miles.component.html',
  styleUrls: ['./stats-rower-miles.component.css']
})
export class StatsRowerMilesComponent implements OnInit {
  ...
  getYearStrokes(year: number) {
    let yearTotal = 0
    this.rower.forEach(entry =>{
      let entryYear = new Date(entry.date).getFullYear()
      if (entryYear == year) {
        yearTotal+= entry.strokes
      }
    })
    return yearTotal
  }
